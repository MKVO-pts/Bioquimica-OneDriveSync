...
def main():
    ...
    check_newFiles(Stored_PATH,NewFiles_PATH,UpdateStored=True)
...
def faster_sha256(path): #924 MB/s
    hashvaluev2 =[]; t_file = 0 ; t_dir = 0 ; size = 0; counter = 0
    for roots, dirs, files in os.walk(path,topdown=True, onerror=print(), followlinks=False):
        for x in files:
            size = os.path.getsize(os.path.join(roots,x))
            hashvaluev2.append(hashlib.sha256(open(os.path.join(roots,x), 'rb').read()).hexdigest())
        t_file += len(files)
        t_dir += len(dirs)
